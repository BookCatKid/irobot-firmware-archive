...
Robot audio is indexed the same way. `data/audio-assets.json` summarizes embedded songs and multilingual voice prompts. At the current snapshot it covers **137,447 unique audio SHA-256s**, **9,148 semantic sound/language combinations**, **52 locale directories**, and **22 robot-song names** across **200 unique parent firmware packages**. The Pages site publishes one representative file per sound under `audio/<sha256>.<ext>` — each clip is directly playable (`<audio controls>`) and downloadable via a plain link, with exact provenance still traceable through the parent firmware manifests/Releases. `scripts/build_audio_packs.py` materializes those static files from the archived firmware (range-download + unsquashfs + hash verify; offline builds write tiny placeholders so links stay valid).
